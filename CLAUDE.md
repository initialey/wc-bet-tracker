...
- src/ai.py : Claude API分析。サッカー用(analyze_match)・汎用(analyze_generic)・MLB用(analyze_mlb: Stats API構造化データを埋込+検索は補助max_uses=3)。根拠は日英。根拠となる事実は最大3点・各1行・専門用語なしの平易な日本語(「どっちが強いか」「最近の調子」「怪我人」レベル。xG等はプロンプトで言い換え指示、確率計算用の数値出力は従来通り)。コード側でもmain.MAX_FACTS=3で切り詰め、表示もdashboard.MAX_FACTS_DISPLAY=3で過去の長い記録を丸める(記録自体は不変)。check_verdict/rewrite_verdict=理由文とピックの整合性ガード用の軽量呼び出し(MODEL_LIGHT=haiku)
- API費用削減(3本柱): ①プロンプトキャッシング — 各analyze_*の固定ルール・出力フォーマットをsystemブロック(cache_control: ephemeral)に分離し、試合ごとに変わるチーム名・オッズ・Stats APIデータはuserメッセージ側(キャッシュ対象外)に分離。同一実行で同種の試合を連続分析するため2件目以降でcache_read_input_tokensがヒットする。②二段階スクリーニング — ai.screen_match(Haiku・検索なし)で市場の暗示勝率(devig後97%以上)やデータ明確な不足(MLB先発投手未発表等)を機械的に判定し、本分析(検索付き・高コスト)に進めるか判定。対象外リーグ・1日の分析上限はconfig.SPORTS/MAX_GAMES_PER_DAYで別途絞り込み済みのためスクリーニングは市場歪み・データ充足の2点のみ。API障害時はproceed=True(安全側)にフォールバックしカバレッジを失わない。見送った試合はdata/screening_log.jsonに理由付きで記録(history.csvは答え合わせ対象の記録専用のため別ファイル、30日超は自動整理)。③検索の絞り込み — Poisson/正規分布の確率計算はすべてsrc/model.py(Python側)で完結しており、AIは事実収集とxG/期待値等の入力数値のみ返す。各analyze_*のmax_usesを引き下げ、検索トピックを必要最小限に限定。usage(cache_read/write等)はai.USAGE_LOGに蓄積し、実行終了時に[cost]ログでモデル別・処理別のトークン内訳を出力(ai.usage_summary())
...
- data/history.csv : 全予想の記録(答え合わせ済み含む)。スキーマ変更時は互換性に注意。prob_raw=補正前確率、bookmaker=最良オッズの提供ブックメーカー名。CLV用の列: odds=賭け時オッズ / created_utc=賭けた時刻 / kickoff_utc=試合開始 / closing_odds=締切オッズ(精密値) / closing_odds_at=取得UTC時刻 / closing_lead_min=取得時点でキックオフまでの分数 / closing_source=live(専用ジョブ)・backfill(historical APIで遡及)・missed(取り逃し、通知済み)・backfill_failed / approx_closing_odds=旧来の近似値(analyze.ymlが毎回の実行で未確定予想の直近観測オッズを上書き)。CLV=記録時オッズ÷締切オッズ−1はclosing_oddsが取れた行のみで集計しダッシュボードに件数(n=)併記。旧CSVは読込時に不足列を空欄で自動補完(closing_odds→approx_closing_odds改名の一回限りマイグレーション含む、冪等)
- .github/workflows/closing_odds.yml + src/closing_odds.py : 締切オッズ取得。GitHubはこのリポジトリのcronを数時間おきにしか起動しない(5分cronでも実測は中央値2.4時間間隔)ため、1回の実行を最大340分の常駐ループ(5分ごとにcapture_once)にし、4時間おきcron+concurrency(後続は待機)で途切れず引き継ぐ(公開リポジトリのためActions時間は無料)。capture_once: キックオフ15分前以内(CAPTURE_LEAD_MIN)の未取得行を種別別の全マーケット(odds_api.CLOSING_MARKETS、422時はコア3市場へフォールバック)で1試合1回取得、キックオフ後5分を過ぎても未取得ならclosing_source=missedにしてTelegramへ通知(取得失敗も通知、同じ試合の失敗は実行内で重複通知しない)。対象なしの反復はAPI呼び出しゼロ。workflow_dispatchのmode: poll(既定)/once(1回)/backfill(historical APIで過去分を遡及取得。--budget クレジット上限・--days 期間、新しい試合から優先。historicalは10クレジット×市場数×リージョン数、コア3市場=30/試合)。ワークフロー失敗時もTelegramへ通知。Secrets: ODDS_API_KEY, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID
- 🎯実弾候補: 実弾テスト用の絞り込み。判定設定はconfig/live_bet.json(コード直書き禁止): min_ev(既定0.03、環境変数LIVE_BET_MIN_EVで上書き可)、exclude_markets=[種別, マーケット]の組(現行: サッカー90分勝敗・ハンディ+0.5・コーナー(参考)、MLB勝敗。回収率が継続してマイナスの市場。ペーパー記録・表示は継続)、go_min_clv/go_min_n=実弾GO条件。ファイルはLIVE_BET_CONFIGで差し替え可、壊れていれば既定値で動作。旧: 確率60%+のみ(MLB×ランライン限定)で判定していたが、オッズを見ないため同じ確率でも価値(EV)が違う予想を区別できなかった。現行はEVベース(記録時のev列=予想確率×オッズ-1)で対象は全スポーツ・全マーケット。ダッシュボードのカテゴリタブ先頭に「🎯 実弾候補」タブ(該当0件はタブ内に案内)、該当カードに損益分岐オッズ(100÷補正後確率)と合格ライン(×LIVE_BET_MARGIN=1.02、2桁切り上げ)を表示し、取得済み最良オッズが合格ライン以上なら✅買い候補/未満なら⚠️要オッズ確認。Slack/Discord通知の冒頭にセクション(0件日は「本日の実弾候補なし」1行)、実績分析のマーケット別成績の先頭に条件(文言は設定ファイルから生成)を過去分へ遡及適用した集計行
- 🚦実弾GO条件: ダッシュボード上部に「CLV +2%以上 かつ n≥200」(config/live_bet.jsonのgo_min_clv/go_min_n)を表示し、市場(スポーツ×マーケット)ごとに 達成/未達/n不足/除外 を色分けチップで表示(dashboard.build内、analytics()のmroiのclv/clv_nを使う)
- ⏱賭けタイミング別成績: main._lead_band で記録時刻→キックオフの時間を 24h+ / 6-24h / <6h に分類し、analytics()のlead_bandsで成績・回収率・CLVを集計、ダッシュボードにカード表示(どの時間帯で賭けると市場に勝てているか)

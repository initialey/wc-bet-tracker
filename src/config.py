...
# --- 通知の円換算表示 ---
# 通知文言は「ユニット/U」を使わず「1回1,000円賭けた場合 +5,290円」の形で提示する。
# 損益は1ユニット=YEN_PER_UNIT円で換算(STAKE=1.0が1回1,000円に相当)
YEN_PER_UNIT = 1000
YEN_STAKE_LABEL = "1回1,000円賭けた場合"


def yen_of(profit_units) -> int:
    """損益(ユニット)を「1回1,000円」換算の金額(円・整数)にする"""
    try:
        return round(float(profit_units or 0) * YEN_PER_UNIT)
    except (TypeError, ValueError):
        return 0


def _pl_mark(yen: int) -> str:
    """損益の絵文字: プラス💰 / マイナス📉 / 増減なし➖"""
    return "💰" if yen > 0 else "📉" if yen < 0 else "➖"


def yen_result_line(profit_units, label: str = YEN_STAKE_LABEL) -> str:
    """損益を絵文字付きの円換算1行にする(例: 「💰 1回1,000円賭けた場合 +5,290円」)"""
    yen = yen_of(profit_units)
    return f"{_pl_mark(yen)} {label} {yen:+,}円"
...
def live_bet_lines(prob):
    """(損益分岐オッズ, 合格ラインオッズ) を返す。
    損益分岐 = 100 ÷ 補正後確率。合格ライン = 損益分岐 × LIVE_BET_MARGIN を
    小数2桁に切り上げ(「この値以上でのみベット」の下限なので安全側に丸める)"""
    import math
    be = 100 / _prob_int(prob)
    # 1e-9は浮動小数の丸め誤差対策(60%→1.70が1.7000000000000002になり1.71へ繰り上がるのを防ぐ)
    return be, math.ceil(be * LIVE_BET_MARGIN * 100 - 1e-9) / 100
